...
class MetricManager:
    ...
    def update_metric_by_context(self, context):
        ...
        for owned_stock in context.basket:
        ...
        self._mdd_list += [_diff_percentage(current_asset,
                                            self._max_total_asset)]
    ...
    def plot_profit_rate(self, comparisons=None):
        ...
        candidates = [
            ('BACK_TEST', np.array(self._asset_list) / self._init_budget)
        ]
        for comparison in comparisons:
            comp_code = self.NAME_TO_CODE[comparison]
            prices = self._feature_manager.get_feature(
                comp_code, 'Close', start=min_market_date, end=max_market_date
            )[self._market_dates].values
